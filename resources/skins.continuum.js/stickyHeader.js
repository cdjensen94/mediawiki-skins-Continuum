...
const
	...
	STICKY_HEADER_ID = 'continuum-sticky-header',
	STICKY_HEADER_APPENDED_ID = '-sticky-header',
	STICKY_HEADER_APPENDED_PARAM = [ 'wvprov', 'sticky-header' ],
	STICKY_HEADER_VISIBLE_CLASS = 'continuum-sticky-header-visible',
	STICKY_HEADER_USER_MENU_CONTAINER_SELECTOR = '.continuum-sticky-header-icon-end .continuum-user-links',
	FIRST_HEADING_ID = 'firstHeading',
	USER_LINKS_DROPDOWN_ID = 'continuum-user-links-dropdown',
	ULS_STICKY_CLASS = 'uls-dialog-sticky',
	ULS_HIDE_CLASS = 'uls-dialog-sticky-hide',
	SEARCH_TOGGLE_SELECTOR = '.continuum-sticky-header-search-toggle',
	STICKY_HEADER_EXPERIMENT_NAME = 'continuum.sticky_header';
...
/**
 * Makes sticky header icons functional for modern Continuum.
 *
...
function prepareIcons( header, history, talk, subject, watch ) {
	...
	if ( watch && watch.parentNode instanceof Element ) {
		...
		const watchIcon = /** @type {HTMLElement} */ ( watchSticky.querySelector( '.continuum-icon' ) );
...
/**
 * Render sticky header edit or protected page icons for modern Continuum.
 *
 * @param {Element} header
...
/**
 * Makes sticky header functional for modern Continuum.
 *
 * @param {Element} header
 * @param {Element} userLinksDropdown
...
function makeStickyHeaderFunctional(
	...
	// When ContinuumPromoteAddTopic is set, #ca-addsection is the link itself
...
function setupSearchIfNeeded( header ) {
	...
	if ( !document.body.classList.contains( 'skin-continuum-search-vue' ) ) {
		return;
	}
