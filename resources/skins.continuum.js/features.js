	...
	if ( !mw.user.isNamed() ) {
		switch ( feature ) {
			...
			case 'appearance-pinned':
				...
				mw.user.clientPrefs.set( `continuum-feature-${ feature }`, enabled ? '1' : '0' );
				break;
			default:
...
function toggleDocClasses( name, override, isNotClientPreference ) {
	...
	const featureClassEnabled = `continuum-feature-${ name }-${ suffixEnabled }`,
		classList = document.documentElement.classList,
		featureClassDisabled = `continuum-feature-${ name }-${ suffixDisabled }`,
...
function getClass( name, featureEnabled, isClientPreference ) {
	if ( featureEnabled ) {
		...
		return `continuum-feature-${ name }-${ suffix }`;
	} else {
