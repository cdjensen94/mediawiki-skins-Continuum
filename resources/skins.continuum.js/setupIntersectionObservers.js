// Enable Continuum features limited to ES6 browse
const
	...
	PAGE_TITLE_INTERSECTION_CLASS = 'continuum-below-page-title';
...
const updateTocLocation = () => {
	...
	const pinnedContainerId = 'continuum-toc-pinned-container';
	const stickyHeaderUnpinnedContainerId = 'continuum-sticky-header-toc-unpinned-container';
	const pageTitlebarUnpinnedContainerId = 'continuum-page-titlebar-toc-unpinned-container';
 ...
 * https://gerrit.wikimedia.org/r/c/mediawiki/skins/Continuum/+/894696/3/resources/common/variables.less ?
 */
...
const setupTableOfContents = ( tocElement, bodyContent, initSectionObserverFn ) => {
		...
		onTogglePinned: () => {
			...
			if ( !features.isEnabled( 'toc-pinned' ) ) {
				...
				const containerSelector = !isStickyHeaderVisible ?
					'.continuum-page-titlebar .continuum-toc-landmark' : '#continuum-sticky-header .continuum-toc-landmark';
				const container = /** @type {HTMLElement} */(
					document.querySelector( containerSelector )
				);
				if ( container ) {
					const containerId = !isStickyHeaderVisible ? 'toc-page-titlebar' : 'toc-sticky-header';
					popupNotification.add( container, mw.message( 'continuum-toc-unpinned-popup' ).text(), containerId )
						.then( ( popupWidget ) => {
							if ( popupWidget ) {
	...
	mw.hook( 'wikipage.tableOfContents' ).add( ( sections ) => {
		tableOfContents.reloadTableOfContents( sections ).then( () => {
			 ...
			 * @since 1.40
			 */
			mw.hook( 'wikipage.tableOfContents.continuum' ).fire( sections );
			updateElements();
		} );
	} );
