<template>
	<cdx-typeahead-search
		:id="id"
		ref="searchForm"
		class="continuum-typeahead-search"
		:class="rootClasses"
	...
	>
		...
		<template #search-results-pending>
			{{ $i18n( 'continuum-search-loader' ).text() }}
		</template>
		<!-- eslint-disable-next-line vue/no-template-shadow -->
		<template #search-footer-text="{ searchQuery }">
			<span v-i18n-html:continuum-searchsuggest-containing="[ searchQuery ]"></span>
		</template>
	</cdx-typeahead-search>
</template>

<script>
const { CdxTypeaheadSearch } = mw.loader.require( 'skins.continuum.search.codex.scripts' ),
...
module.exports = exports = defineComponent( {
	...
	computed: {
		rootClasses() {
			return {
				'continuum-search-box-disable-transitions': this.disableTransitions,
				'continuum-typeahead-search--active': this.isFocused
			};
		},
		visibleItemLimit() {
