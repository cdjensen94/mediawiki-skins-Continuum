const portlets = require( '../../../resources/skins.continuum.js/portlets.js' );
const mustache = require( 'mustache' );
const fs = require( 'fs' );
const menuTemplate = fs.readFileSync( 'includes/templates/Continuum/Menu.mustache', 'utf8' );
const menuContentsTemplate = fs.readFileSync( 'includes/templates/Continuum/MenuContents.mustache', 'utf8' );
const dropdownOpen = fs.readFileSync( 'includes/templates/Continuum/Dropdown/Open.mustache', 'utf8' );
const dropdownClose = fs.readFileSync( 'includes/templates/Continuum/Dropdown/Close.mustache', 'utf8' );

describe( 'portlets', () => {
	beforeEach( () => {
		const marker = document.querySelector( '#continuum-page-tools-dropdown' );
		if ( marker ) {
			marker.remove();
		}
	} );
	...
	test( 'portlets that go through the hook method should match the menu template HTML (dropdowns)', () => {
		...
		marker.id = 'continuum-page-tools-dropdown';
		...
		const dropdown = transformedPortlet.closest( '.continuum-dropdown' );
