...
const fs = require( 'fs' );
const tableOfContentsTemplate = fs.readFileSync( 'includes/templates/Continuum/TableOfContents.mustache', 'utf8' );
const tableOfContentsContentsTemplate = fs.readFileSync( 'includes/templates/Continuum/TableOfContents__list.mustache', 'utf8' );
const tableOfContentsLineTemplate = fs.readFileSync( 'includes/templates/Continuum/TableOfContents__line.mustache', 'utf8' );
...
const initTableOfContents = require( '../../resources/skins.continuum.js/tableOfContents.js' );
...
const SECTIONS = [
	{
		...
		'continuum-button-label': 'Toggle bar subsection',
		'array-sections': [ {
			toclevel: 2,
			number: '2.1',
			line: 'baz',
...
function render( templateProps = {} ) {
	const templateData = Object.assign( {
		'msg-continuum-toc-beginning': 'Beginning',
...
function mount( templateProps = {} ) {
	...
	container = /** @type {HTMLElement} */ ( document.getElementById( 'continuum-toc' ) );
...
describe( 'Table of contents', () => {
	...
	describe( 'renders', () => {
		test( 'when `continuum-is-collapse-sections-enabled` is false', () => {
			toc = mount();
			expect( document.body.innerHTML ).toMatchSnapshot();
			expect( barSection.classList.contains( toc.EXPANDED_SECTION_CLASS ) ).toEqual( true );
		} );
		test( 'when `continuum-is-collapse-sections-enabled` is true', () => {
			toc = mount( { 'continuum-is-collapse-sections-enabled': true } );
			expect( document.body.innerHTML ).toMatchSnapshot();
