const portlets = require( '../../../resources/skins.continuum.legacy.js/portlets.js' );
const mustache = require( 'mustache' );
const fs = require( 'fs' );
const menuTemplate = fs.readFileSync( 'includes/templates/Continuum/LegacyMenu.mustache', 'utf8' );
...
describe( 'portlets', () => {
	test( 'portlets that go through the hook method should match the menu template HTML (non-dropdown version)', () => {
		...
		const expected = portletHTML.replace( /[\s\n]/gi, '' )
			...
			// rather than `h3 > span`
			.replace( /<spanclass="continuum-menu-heading-label">/g, '' )
			.replace( /<h3/g, '<div' )
	...
	test( 'portlet dropdowns that go through the hook method should match the menu template HTML (dropdown version)', () => {
		...
		const label = 'label text';
		const portletHTML = mustache.render( menuTemplate, {
			id, class: 'continuum-menu-dropdown', label,
			'is-dropdown': true
		}, {
			MenuContents: menuContentsTemplate
		} );
