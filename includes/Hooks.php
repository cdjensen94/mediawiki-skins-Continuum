	...
	SkinPageReadyConfigHook
{
	...
	private static function isContinuumSkin( string $skinName ): bool {
		return (
			...
			$skinName === Constants::SKIN_NAME_DARK
		);
	}
	...
	private static function updateViewsMenuIcons( &$content_navigation, $isLegacy ) {
		...
		foreach ( $content_navigation['views'] as &$item ) {
			...
			if ( $icon ) {
				...
				// The continuum-tab-noicon class is only used in Continuum.
				self::appendClassToItem(
					$item['class'],
	 ...
	 * This can possibly be revised based on the outcome of T2876.
	 *
	...
	public static function onSkinTemplateNavigation( $sk, &$content_navigation ) {
		...
		if ( $skinName === Constants::SKIN_NAME_DARK ) {
	...
	public function onGetPreferences( $user, &$prefs ): void {
		...
		$continuumPrefs = [
			Constants::PREF_KEY_LIMITED_WIDTH => [
				...
				'hide-if' => [ '!==', 'skin', Constants::SKIN_NAME_DARK ],
			],
		...
		if ( $default ) {
			$this->userOptionsManager->setOption(
				...
				$default === Constants::SKIN_VERSION_LEGACY ?
					Constants::SKIN_NAME_LEGACY : Constants::SKIN_NAME_DARK
			);
		}
	}
	 ...
	 *
	 * @param string $skinName hint that can be used to detect modern continuum.
	 * @return bool
	 */
	...
	/**
	 * Register Continuum  beta feature to the beta features list
	 *
	...
	public function onGetBetaFeaturePreferences( User $user, array &$betaFeatures ) {
		...
		// Only Continuum  is supported for beta features
		if ( $skinName !== Constants::SKIN_NAME_DARK ) {
			return;
		}
		// Only add Continuum  beta feature if there is at least one beta feature present in config
		$configHasBeta = false;
		...
		$betaFeatures[ Constants::VECTOR__BETA_KEY ] = [
